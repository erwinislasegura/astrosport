...
final class ShopCart
{
    ...
    public static function addPack(int $setId, array $photoIds): void
    {
        ...
        $keys = array_values(array_filter(self::keys(), fn($key) => !str_starts_with($key, 'pack:'.$setId.':')));
    ...
    {
        $_SESSION['cart'] = array_values(array_filter(self::keys(), fn($value) => $value !== $key));
    }
    ...
    public static function items(): array
    {
        ...
        foreach (self::keys() as $key) {
            ...
            if ($type === 'photo') {
                ...
                if ($photo && ($photo['individual_enabled'] ?? 1)) {
                    $items[] = ['key' => $key, 'type' => 'photo', 'id' => (int) $photo['id'], 'photo_id' => (int) $photo['id'], 'set_id' => null, 'selected_photo_ids' => [], 'title' => $photo['title'], 'event_name' => $photo['event_name'], 'price' => (int) $photo['price'], 'preview_path' => $photo['preview_path']];
                }
                continue;
            }
            ...
            if ($type === 'pack') {
                ...
                $validIds = array_map('intval', array_column(array_filter($photos, fn($photo) => (int) $photo['set_id'] === (int) $set['id']), 'id'));
                sort($validIds, SORT_NUMERIC);
                $option = PhotoPack::matching((int) $set['id'], count($ids));
                if (!$option || $ids !== $validIds) continue;
                $cover = $photos[0] ?? null;
                $items[] = ['key' => $key, 'type' => 'pack', 'id' => (int) $set['id'], 'photo_id' => null, 'set_id' => (int) $set['id'], 'selected_photo_ids' => $ids, 'title' => 'Combo de '.count($ids).' fotos · '.$set['name'], 'event_name' => $set['event_name'], 'price' => (int) $option['price'], 'preview_path' => $cover['preview_path'] ?? $set['preview_path'], 'cover_id' => $cover['id'] ?? $set['cover_id'], 'photos_count' => count($ids)];
            }
        }
        return $items;
    }
