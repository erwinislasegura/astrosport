...
final class OrderMailer
{
    public function sendPaid(int $orderId): bool
    {
        ...
        try{$sent=(new MailTransport())->send($order['customer_email'],$subject,$html);}catch(\Throwable $e){$sent=false;error_log('AstroSport SMTP pedido '.$orderId.': '.$e->getMessage());}
        if(!$sent){$this->release($orderId);error_log('AstroSport email: no fue posible enviar el pedido '.$orderId);}
        return $sent;
    }
