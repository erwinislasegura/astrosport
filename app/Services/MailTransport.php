...
final class MailTransport
{
    public function send(string $to,string $subject,string $html):bool
    {
        ...
        $mail=config('mail')?:[];$from=$mail['from_email']??'no-reply@astrosport.cl';$name=$mail['from_name']??'AstroSport';
        $headers=['MIME-Version: 1.0','Content-Type: text/html; charset=UTF-8','From: '.$this->encoded($name).' <'.$from.'>','Reply-To: '.($mail['reply_to']??$from),'X-Mailer: AstroSport'];
