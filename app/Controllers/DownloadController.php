...
final class DownloadController
{
    ...
    {
        if(!class_exists(ZipArchive::class)){http_response_code(503);exit('El servidor debe habilitar ZipArchive para descargar sets.');}
        if(!$photos)$this->denied();$tmp=tempnam(sys_get_temp_dir(),'astrosport-set-');$zip=new ZipArchive();if($zip->open($tmp,ZipArchive::CREATE|ZipArchive::OVERWRITE)!==true)$this->denied();$root=realpath(ROOT.'/storage/originals');
        ...
        $zip->close();$safe=trim(preg_replace('/[^a-z0-9-]+/i','-',strtolower($name)),'-')?:'set-astrosport';header('Content-Type: application/zip');header('Content-Disposition: attachment; filename="'.$safe.'.zip"');header('Content-Length: '.filesize($tmp));$this->headers();readfile($tmp);@unlink($tmp);exit;
    }
