...
<section class="event-sets-section section" id="sets-evento">
 <div class="event-sets-head"><div><span class="eyebrow dark">COLECCIÓN DEL EVENTO</span><h2>ELIGE TU <em>SET.</em></h2><p>Cada set puede incluir compra individual, combos por cantidad o descarga completa.</p></div><b><?=count($sets)?> SETS PUBLICADOS</b></div>
 <div class="event-public-set-grid">
  ...
  <article>
   <a class="event-set-cover" href="<?=url('foto?id='.$set['cover_id'])?>">
    ...
    <span>ASTROSPORT</span><b><?=$set['photos_count']?> FOTOS</b>
   </a>
   <div class="event-set-details">
    ...
    <div class="event-sale-modes"><?php if($set['individual_enabled']):?><span>INDIVIDUAL</span><?php endif;?><?php if(!empty($set['pack_enabled'])):?><span>COMBOS</span><?php endif;?><?php if($set['set_enabled']):?><span>SET COMPLETO</span><?php endif;?></div>
    <div class="event-set-price"><span>DESDE <b><?=money((int)($set['individual_enabled']?$set['individual_price']:(!empty($set['pack_enabled'])?$set['pack_price']:$set['set_price'])))?></b></span><a href="<?=url('foto?id='.$set['cover_id'])?>">VER FOTOGRAFÍAS →</a></div>
   </div>
  </article>
  <?php endforeach;?>
 </div>
