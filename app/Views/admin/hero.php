...
<div class="content">
 ...
 <div class="hero-admin-grid">
  <form class="panel hero-form" method="post" enctype="multipart/form-data" id="heroForm"><input type="hidden" name="_token" value="<?=csrf()?>">
   ...
   <div class="two-fields"><label>TEXTO DEL BUSCADOR<input name="search_placeholder" maxlength="160" value="<?=htmlspecialchars($hero['search_placeholder'])?>"></label><label>TEXTO DEL BOTÓN<input name="button_text" maxlength="80" value="<?=htmlspecialchars($hero['button_text'])?>"></label></div>
   <div class="hero-media-settings"><label>TIPO DE FONDO<select name="media_type" id="heroMediaType"><option value="image" <?=$hero['media_type']==='image'?'selected':''?>>Imagen configurada</option><option value="local_video" <?=$hero['media_type']==='local_video'?'selected':''?>>Video MP4 local o externo</option><option value="youtube" <?=$hero['media_type']==='youtube'?'selected':''?>>Video de YouTube</option></select></label><label>URL O RUTA DEL VIDEO<input name="video_url" id="heroVideoUrl" maxlength="500" value="<?=htmlspecialchars($hero['video_url'])?>" placeholder="https://www.youtube.com/watch?v=… o assets/video.mp4"><small>Para YouTube se inicia automáticamente en el segundo 30, sin controles.</small></label></div>
